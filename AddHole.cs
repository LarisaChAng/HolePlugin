...
namespace HolePlugin
{
    ...
    public class AddHole : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            Document ovDoc = arDoc.Application.Documents.OfType<Document>().Where(x => x.Title.Contains("OV")).FirstOrDefault();
            if (ovDoc == null)
            {
                TaskDialog.Show("Ошибка", "Не найден ОВ (OV) файл");
                return Result.Cancelled;
            }

            //проверка наличия семейства "Отверстие"
            ...
            transaction.Start("Рассановка отверстий");
            ...
            foreach (Duct d in ducts)
            {
                ...
                foreach (ReferenceWithContext refer in intersections)
                {
                    ...
                    //Вставка экземпляра семейства "Отверстие"
